...
export const AudioProvider: React.FC<{ children: React.ReactNode, initialReciter: Reciter | null }> = ({ children, initialReciter }) => {
  ...
  const [nowPlaying, setNowPlaying] = useState<Surah | null>(() => getLocalItem<Surah | null>('quran_nowPlaying', null));
  const [currentReciter, setCurrentReciter] = useState<Reciter | null>(initialReciter);
  const [currentTime, setCurrentTime] = useState(() => getLocalNumber('quran_currentTime', 0));
  const [duration, setDuration] = useState(0);
  const [volume, setVolume] = useState(() => getLocalNumber('quran_volume', 0.7));
  const [playbackSpeed, setPlaybackSpeed] = useState(1);

  const [surahProgress, setSurahProgress] = useState<SurahProgress>(() => getLocalItem<SurahProgress>('quran_progress', {}));
  const [userStats, setUserStats] = useState<UserStats>(() => getLocalItem<UserStats>('quran_stats', { totalListenTime: 0, surahsStarted: [], surahsCompleted: [] }));
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && 'mediaSession' in navigator && nowPlaying && currentReciter) {
